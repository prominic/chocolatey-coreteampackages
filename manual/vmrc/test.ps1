...
if ($env:APPVEYOR_BUILD_VERSION) {
  ...
  # run manually
  [xml]$spec = Get-Content vmrc.nuspec
  $version = $spec.package.metadata.version
}

"TEST: Version $version in vmrc.nuspec file should match"
[xml]$spec = Get-Content vmrc.nuspec
...
$zip = [IO.Compression.ZipFile]::OpenRead("$pwd\vmrc.$version.nupkg")
...
. choco install -y vmrc -source . -version $version

"TEST: Uninstallation of package should work"
. choco uninstall -y vmrc -source .
